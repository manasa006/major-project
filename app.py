...
conn=mysql.connector.connect(host='localhost',username='root',password='Bhaskar@123',database='loan')
...
def contact():
    if request.method ==  'POST':
        ...
        my_cursor.execute("insert into contact values(%s,%s,%s,%s)",(name,phno,email,message))
        conn.commit()
